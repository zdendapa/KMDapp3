    ...
    database.transaction(function(tx){
        tx.executeSql('SELECT * FROM sheetsdata WHERE shid="'+shid+'"', [], function(tx, results) {
                ...
                for (var i=0; i<len; i++){
                    ...
                    $("ul.content").append('<li data-id="'+results.rows.item(i).rowid+'"><span class="dater"><input onchange="dateFormatCheck(this)" onfocus="dateFormatIn(this)" value="'+results.rows.item(i).dater+'"></span><span class="description"><input  value="'+xmlSpecCharEn2(results.rows.item(i).desc)+'" onchange="addRowCheck(this)"></span><span class="paid"><select>'+dbHowPaidOptionsHtml+'</select></span><span class="checkRef"><input value="'+results.rows.item(i).checkRef+'"></span> <span class="payment"><input value="'+results.rows.item(i).payment+'"></span> <span class="last"><input style="color:'+(results.rows.item(i).balance>=0?String("black"):String("red"))+'"  value="'+results.rows.item(i).balance+'" readonly></span> </li>');
